...
function renderContacts(contactsArray) {
    ulElement.innerHTML = ""; // Borrar todos los elementos dentro del <ul>
    let i=0;
...
function catchEvent(event){
    ...
    .then(response => response.json())
    .then(info => {
        data = info; // Asignar los datos a la variable 'datos'
        //return datos;        
        let filteredData=[];

        for (let i=0; i<data.length; i++){
            //console.log(searchingInput.value.toLocaleLowerCase());
            //console.log(data[i].name.substring(0,searchingInput.length).toLowerCase());
